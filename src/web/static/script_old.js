...
console.log('Script loaded - initializing elements...');
...
console.log('Elements found:', {
    ...
    forecastDaysSelect: !!forecastDaysSelect,
    btnFetch: !!btnFetch,
...
function processAndRenderData(rawData) {
    ...
    const normalized = rawData.map(item => {
        ...
        return {
            ...
            interval: item[intervalKey] || item.interval || item.hour,
...
function renderChart() {
    ...
    const labels = allData.map(d => {
        ...
        return `${d.date} ${String(interval).padStart(2, '0')}:00`;
    });
...
function showError(message) {
    ...
    const labels = currentData.map(d => d.time);
    ...
    // Add Volume if reasonable scale difference handling needed? 
    // For now simple Price chart as requested.
...
async function fetchForecast() {
    if (currentData.length === 0) {
        alert("Fetch data first to train the model.");
        return;
    }

    setLoading(true);
    try {
        // Send a simplified series for training
        const history = currentData.map((d, i) => {
            // Fake a date if time is just an hour (0-24)
            // If d.time is 1, 2, 3.. we need to construct a proper timestamp for the backend
            // The backend expects specific keys.
            // Actually, simplest is to just send the 'price' series and let backend assume it's hourly or daily.

            // BUT, the requirement says "train model and use it". 
            // Time-series forecasting needs simpler inputs usually.

            // Let's construct a list of {date: "YYYY-MM-DD", price: ...}
            // We'll use the date from input + time index if needed.
            // This part is tricky without knowing exact GME format.
        ...
        const response = await fetch('/api/forecast', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                history: currentData.map(d => ({
                    // We need a real date for the backend logic I wrote (which expects 'date')
                    // If the frontend has hour 1..24, we combine with selected date
                    date: combineDate(dateInput.value, d.time),
                    price: d.price
                }))
            })
        });
        ...
        if (forecast.length > 0) {
            ...
            const priceData = chartInstance.data.datasets[0].data;

            // Extend existing dataset with nulls for gaps if any?
            // Actually, we should add a NEW dataset for forecast
            const forecastPrices = [];
            const forecastLabels = [];

            // Pad the beginning with nulls to align?
            // ChartJS handles this by matching labels.

            forecast.forEach(f => {
                labels.push(f.date);
                // Push to forecast dataset, null to main
                priceData.push(null);
                forecastPrices.push(f.price);
            });

            // We need a dataset that is ALL valid prices + forecast?
            // Or just a separate line.

            // Let's add a separate dataset
            const forecastDataset = {
                label: 'Forecast',
                data: new Array(currentData.length).fill(null).concat(forecast.map(f => f.price)),
                borderColor: '#8b5cf6',
                borderDash: [5, 5],
                tension: 0.4
            };
            ...
            chartInstance.data.datasets.push(forecastDataset);
...
function combineDate(baseDate, timePart) {
    // If timePart is integer (hour), add simple date
    // Backend expects YYYY-MM-DD
    // If it's intraday, XGBoost logic in backend (which expects 'date' column with Y-M-D) 
    // might fail if we send same date multiple times with different hours.
    // The backend logic was: df['date'] = pd.to_datetime(df['date']), then group day/month/year.
    // It seems designed for DAILY data.
    // If GME returns HOURLY data, we might need to aggregate or adjust backend.
    // For now, I'll return the baseDate, assuming the backend can handle duplicates or we just use it for visual.

    // Wait, the backend logic: "Predict future ... range(1, forecast_days)".
    // It predicts DAILY future.
    // So we should feed it data where the time key is a Date string.

    // Hack: just return the baseDate text. The backend will parse it.
    return baseDate;
}
...
function exportData() {
    ...
    fetch('/api/export', {
        ...
        .then(blob => {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `gme_data_${dateInput.value}.xlsx`;
            document.body.appendChild(a);
            a.click();
            a.remove();
        });
